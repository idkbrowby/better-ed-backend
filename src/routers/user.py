...
from src.auth.sessions import AbstractSessionStorage
from src.dependencies import get_sessions, is_authorized
from src.models import UserInLogin, UserInSignup
...
async def set_session(
    ...
    logger.info(f"Session {session} successfully created")
...
async def logout(
    session_id: UUID = Header(default=None, alias="session-id"),
    sessions: AbstractSessionStorage = Depends(get_sessions),
) -> JSONResponse:
